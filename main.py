...
def calculate_ssim_score(video_a_path,
    ...
    for idx, (frame_a,
        ...
        if idx % n_skip_frames != 0:
            continue
        ssim_results.append(calculate_frame_similarity(frame_a, frame_b))
...
def main():
    ...
    plt.hlines([np.average(scores)], xmin=frames[0], xmax=frames[-1], colors = ["red"], label="Average SSIM score over range")
    plt.legend()
    plt.show()
